...
services:
  ...
  tailforwarder-exit: # Scenario: Exit Node for routing internet traffic
    ...
    environment:
      ...
      - TS_USERSPACE=false
      - TS_STATE_DIR=/var/lib/tailscale
      - TS_ACCEPT_DNS=true # Optional: Accept Tailscale DNS to reduce log noise
...
*   `ENABLE_EXIT_NODE`: Set to `true` to configure the container as a Tailscale exit node.
    ...
    *   `TS_ACCEPT_DNS`: Optionally set to `true` to accept Tailscale DNS and reduce DNS-related log noise from Tailscale clients.
